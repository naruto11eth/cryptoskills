...
MegaETH created `realtime_sendRawTransaction` first. Later, `eth_sendRawTransactionSync` was standardized as EIP-7966. MegaETH proxies both -- they are functionally identical. Use `eth_sendRawTransactionSync` for cross-chain compatibility.
...
Multicall is the preferred approach for batching `eth_call` requests.

### Why Multicall?

With v2.0.14, `eth_call` is 2-10x faster. The EVM execution time is a small fraction of total CPU time -- most overhead is per-RPC framework cost. Multicall amortizes this overhead across multiple calls.
...
Earlier MegaETH guidance recommended JSON-RPC batching over Multicall because Multicall made it difficult to cache `eth_call` results. With v2.0.14's performance improvements, the caching mechanism is deprecated and Multicall is now preferred.

### Avoid

- Mixing slow + fast methods: do not batch `eth_getLogs` with `eth_call`; logs are always slower
- Blocking UX on historical queries: keep `eth_getLogs` in background

## Rate Limiting

Public endpoints have rate limits based on CPU time consumption and network bandwidth (32 MB/s average, 15x burst for VIP).
...
1. Warm up connections: call `eth_chainId` on app init to pre-establish HTTP connection
2. Use Multicall for eth_call batching (v2.0.14+)
3. Do not batch slow with fast: `eth_getLogs` blocks entire batch response
...
### Benchmarks (West Coast US to Public RPC)
...
```typescript
const ws = new WebSocket('wss://mainnet.megaeth.com/ws');
ws.send(JSON.stringify({ method: 'eth_sendRawTransactionSync', params: [signedTx], jsonrpc: '2.0', id: 1 }));
```

### 2. Pre-Sign Transactions

Do not sign in the hot path. Prepare transactions ahead of time:
...
Do not wait for confirmations between transactions:
...
```bash
cast estimate --from 0xYourAddress --value 0.001ether 0xRecipient \
